// D:\Projects\Kalwanga\packages\backend\src\controllers\inventoryController.ts
...
/**
 * Get business unit ID from user or request, with database fallback
 * Fetches ALL business units if no specific ID is provided
 */
async function getBusinessUnitId(req: Request): Promise<string> {
  ...
  const sanitizedId = sanitizeBusinessUnitId(businessUnitId);
  ...
  if (!sanitizedId) {
    try {
      if (user?.id) {
        ...
        if (userWithBusinessUnits && userWithBusinessUnits.businessUnits) {
          const firstBusinessUnit = userWithBusinessUnits.businessUnits[0];
          if (firstBusinessUnit && firstBusinessUnit.businessUnit && firstBusinessUnit.businessUnit.isActive) {
            const bu = firstBusinessUnit.businessUnit;
            console.log('✅ Using business unit from user:', bu.id, bu.name);
            return bu.id;
          }
        }
      }
      ...
      if (businessUnits.length > 0) {
        console.log('✅ Using first active business unit from DB:', businessUnits[0].id, businessUnits[0].name);
  ...
  if (!existing || !existing.isActive) {
    console.warn(`⚠️ Business unit ${sanitizedId} not found or inactive, falling back to first active`);
    const businessUnits = await prisma.businessUnit.findMany({
      where: { isActive: true },
      orderBy: { createdAt: 'asc' },
      take: 1,
    ...
    if (businessUnits.length > 0) {
      console.log('✅ Using first active business unit as fallback:', businessUnits[0].id, businessUnits[0].name);
      return businessUnits[0].id;
    }
    
    const newReq = { ...req } as Request;
    newReq.body = { ...req.body, businessUnitId: 'default' };
    return getBusinessUnitId(newReq);
  }
...
/**
 * Get all business units from the database
 */
...
/**
 * Get user ID from request, with database fallback
 */
...
/**
 * Get company ID from user or request, with database fallback
 */
async function getCompanyId(req: Request): Promise<string> {
  const user = (req as any).user;
  
  let companyId = user?.companyId || req.body?.companyId || req.query?.companyId;
...
export const inventoryController = {
  // ============================================
  // REFERENCE DATA ENDPOINTS (Categories & Suppliers)
  // ============================================
      ...
      if (!businessUnitId || businessUnitId === 'default' || businessUnitId === 'default-business-unit') {
        ...
        if (user?.businessUnits && user.businessUnits.length > 0) {
          ...
          if (firstBU) {
            ...
            console.log(`✅ Using first active business unit from DB: ${businessUnitId}`);
          }
        }
      }
      
      console.log(`📤 Calling inventoryService.getCategories with businessUnitId: ${businessUnitId || 'undefined'}`);
      
      const categories = await inventoryService.getCategories(businessUnitId);
      
      console.log(`📥 inventoryService.getCategories returned ${categories.length} categories`);
  ...
  async getInventory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const { page, limit, search, lowStock, productId, category, location, status, sortBy, 
        sortOrder, includeInactive, hasBarcode, minPrice, maxPrice, supplier, inStock } = req.query;
      ...
      console.log('📤 Calling inventoryService.getInventory with params:', JSON.stringify(params, null, 2));

      const result = await inventoryService.getInventory(params) as InventoryResponse;

      const normalizedInventory = normalizeInventoryItems(result?.inventory || []);
  ...
  async getInventoryByProduct(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const inventory = await inventoryService.getInventoryByProduct(productId, businessUnitId);
  ...
  async getLowStockItems(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(200).json({
        ...
        data: normalizedItems,
        count: normalizedItems.length,
        message: normalizedItems.length > 0 ? `${normalizedItems.length} items are low on stock` : 'No low stock items',
      });
    } catch (error) {
      return handleGeneralError(error, res);
    }
  ...
  async getOutOfStockItems(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      res.status(200).json({
        ...
        data: normalizedItems,
        count: normalizedItems.length,
        message: normalizedItems.length > 0 ? `${normalizedItems.length} items are out of stock` : 'No out of stock items',
      });
    } catch (error) {
      return handleGeneralError(error, res);
    }
  ...
  async getInventoryTransactions(req: Request, res: Response, next: NextFunction) {
    try {
      const businessUnitId = await getBusinessUnitId(req);
      const { page, limit, productId, transactionType, startDate, endDate, variantId } = req.query;
  ...
  async getInventoryByLocation(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const items = await inventoryService.getInventoryByLocation(location, businessUnitId);
      const normalizedItems = normalizeInventoryItems(items);
  ...
  async getInventoryByCategory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const items = await inventoryService.getInventoryByCategory(category, businessUnitId);
      const normalizedItems = normalizeInventoryItems(items);
  ...
  async exportInventory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await inventoryService.exportInventory(businessUnitId, format as string);
  ...
  async getInventorySummary(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const [totalItems, lowStockItems, outOfStockItems, totalValue, categories] = await Promise.all([
    ...
    try {
      ...
      if (!businessUnitId || businessUnitId === 'default' || businessUnitId === 'default-business-unit') {
        ...
        if (user?.businessUnitId) {
          ...
          businessUnitId = user.businessUnits[0].businessUnitId || user.businessUnits[0].id;
        } else {
          const firstBU = await prisma.businessUnit.findFirst({
            where: { isActive: true },
            select: { id: true },
      ...
      console.log(`📤 Calling inventoryService.getCategorySummary with businessUnitId: ${businessUnitId || 'undefined'}`);
      
      const categories = await inventoryService.getCategorySummary(businessUnitId || '');
      
      console.log(`📥 inventoryService.getCategorySummary returned ${categories.length} categories`);
  ...
  async getInventoryByBarcode(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await inventoryService.getInventoryByBarcode(barcode, businessUnitId);
  ...
  async getInventoryBySku(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await inventoryService.getInventoryBySku(sku, businessUnitId);
  ...
  async generateInventoryBarcode(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await inventoryService.generateInventoryBarcode(id, businessUnitId);
  ...
  async generateInventoryQRCode(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await inventoryService.generateInventoryQRCode(id, businessUnitId);
   ...
   */
  async bulkGenerateInventoryBarcodes(req: Request, res: Response, next: NextFunction) {
    try {
      const { ids } = req.body;
      const businessUnitId = await getBusinessUnitId(req);
      ...
      const result = await inventoryService.bulkGenerateInventoryBarcodes(ids, businessUnitId);
  ...
  async scanInventory(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      if (barcode) {
        result = await inventoryService.getInventoryByBarcode(barcode, businessUnitId);
      }
      ...
      if (!result && productId) {
        result = await inventoryService.getInventoryByProduct(productId, businessUnitId);
      }
      ...
      if (normalizedResult.barcode) {
        barcodeInfo = {
          ...
          barcodeUrl: `https://barcode.tec-it.com/barcode.ashx?data=${encodeURIComponent(normalizedResult.barcode)}&code=EAN-13&dpi=96`,
      ...
      res.status(200).json({
        ...
        data: {
          ...
          availableStock: (normalizedResult.stock || 0) - (normalizedResult.reserved || 0),
          barcodeInfo,
        },
  ...
  async createItem(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const itemData = {
        ...
        ...(validatedData.isDigital !== undefined && { isDigital: validatedData.isDigital }),
  ...
  async deleteItem(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await inventoryService.deleteProduct(id, businessUnitId, userId);
  ...
  async createProduct(req: Request, res: Response, next: NextFunction) {
    try {
      ...
      const result = await inventoryService.createProductWithInventory({
        ...
        price: typeof validatedData.price === 'number' ? validatedData.price : (typeof validatedData.unitPrice === 'number' ? validatedData.unitPrice : 0),
        unitPrice: typeof validatedData.unitPrice === 'number' ? validatedData.unitPrice : undefined,
        costPrice: typeof validatedData.costPrice === 'number' ? validatedData.costPrice : undefined,
        stock: typeof validatedData.stock === 'number' ? validatedData.stock : 0,
        reorderPoint: typeof validatedData.reorderPoint === 'number' ? validatedData.reorderPoint : undefined,
        category: typeof validatedData.category === 'string' ? validatedData.category : undefined,
        categoryId: typeof validatedData.categoryId === 'string' ? validatedData.categoryId : undefined,
        location: typeof validatedData.location === 'string' ? validatedData.location : undefined,
        barcode: typeof validatedData.barcode === 'string' ? validatedData.barcode : undefined,
        description: typeof validatedData.description === 'string' ? validatedData.description : undefined,
        images: Array.isArray(validatedData.images) ? validatedData.images : undefined,
        supplier: typeof validatedData.supplier === 'string' ? validatedData.supplier : undefined,
        supplierId: typeof validatedData.supplierId === 'string' ? validatedData.supplierId : undefined,
        expiryDate: validatedData.expiryDate ? new Date(String(validatedData.expiryDate)) : undefined,
        batchNumber: typeof validatedData.batchNumber === 'string' ? validatedData.batchNumber : undefined,
        businessUnitId,
        userId,
      });
  ...
  async transferStock(req: Request, res: Response, next: NextFunction) {
    try {
      const businessUnitId = await getBusinessUnitId(req);
      const userId = await getUserId(req);
      const { productId, fromLocation, toLocation, quantity, notes, variantId } = req.body;

      if (!productId || !fromLocation || !toLocation || !quantity) {
        throw new AppError('Product ID, fromLocation, toLocation, and quantity are required', 400);
      }
    ...
    try {
      ...
      for (const item of validatedData.items) {
        try {
          ...
          results.push({ success: true, data: normalizeInventoryItem(result) });
        } catch (error) {
          errors.push({
            success: false,
            item: item.name,
      ...
      res.status(201).json({
        ...
        data: {
          ...
          summary: {
            ...
            successRate: ((results.length / validatedData.items.length) * 100).toFixed(2) + '%',
          },
        },
    ...
    try {
      ...
      for (const update of validatedData.updates) {
        try {
          ...
          results.push({ id: update.id, success: true, data: normalizeInventoryItem(result) });
        } catch (error) {
          errors.push({ 
            id: update.id, 
            success: false, 
      ...
      res.status(200).json({
        ...
        data: { 
          ...
          summary: { 
            ...
            succeeded: results.length, 
            failed: errors.length,
            successRate: ((results.length / validatedData.updates.length) * 100).toFixed(2) + '%',
          } 
        },
    ...
    try {
      ...
      if (format && typeof format === 'string') {
        if (validFormats.includes(format as ExportFormat)) {
          ...
          console.warn(`⚠️ Invalid export format: ${format}, using 'json' as default`);
        }
      }

      const result = await inventoryService.exportInventoryToFile(businessUnitId, exportFormat);

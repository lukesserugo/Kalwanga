...
export function setupIPC(mainWindow: BrowserWindow | null, store: Store): void {
  // Bind the scanner coordinator to the same window so it can
  // forward serial-scan events to the renderer.
  attachScannerWindow(mainWindow);  
  // ============================================
  // WINDOW CONTROLS
  // ============================================
  ...
  ipcMain.handle('sync:status', () => {
    try {
      ...
      const pendingRow = db
        .prepare("SELECT COUNT(*) as count FROM offline_queue WHERE status = 'pending'")
        .get();
      const failedRow = db
        .prepare("SELECT COUNT(*) as count FROM offline_queue WHERE status = 'failed'")
        .get();
      const lastSyncRow = db
        .prepare("SELECT value FROM settings WHERE key = 'last_sync'")
        .get();

      return {
        pending: pendingRow ? (pendingRow as { count: number }).count : 0,
        failed: failedRow ? (failedRow as { count: number }).count : 0,
        lastSync: lastSyncRow ? (lastSyncRow as { value: string }).value : null,
        isSyncing: syncService.isSyncing || false,
        autoSync: (store.get('autoSync') as boolean) ?? true,
        syncInterval: (store.get('syncInterval') as number) ?? 300,
      };
    } catch (error) {
  ...
  logger.info('IPC handlers setup complete');
    ...
    try {
      ...
      logger.error('Failed to list serial devices:', error);
      return [];
    }
  });

  ipcMain.handle('scanner:request-device', async () => {
    try {
      return await requestSerialDevice();
    } catch (error) {
      logger.error('Failed to request serial device:', error);
      throw error;
    }
  });

  ipcMain.handle('scanner:connect', async (_event, deviceId: string) => {
    if (!deviceId || typeof deviceId !== 'string') {
      throw new Error('deviceId is required');
    }
    return connectSerialDevice(deviceId);
  });

  ipcMain.handle('scanner:disconnect', async (_event, deviceId: string) => {
    if (!deviceId || typeof deviceId !== 'string') {
      throw new Error('deviceId is required');
    }
    return disconnectSerialDevice(deviceId);
  });

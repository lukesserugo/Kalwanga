...
import {
  ...
  Users, Package, DollarSign, ShoppingBag, TrendingUp, Loader2,
  CheckCircle, XCircle, Clock, Calendar
} from 'lucide-react';
import { businessUnitService, BusinessUnitStats } from '../../services/businessUnitService';
...
// FIXED: Use Omit to avoid company type conflict
interface BusinessUnitWithDetails extends Omit<BusinessUnit, 'company'> {
  _count?: {
    products?: number;
    inventory?: number;
    sales?: number;
...
export function BusinessUnitDetail({ id }: BusinessUnitDetailProps) {
  ...
  const handleDelete = async () => {
    ...
    const confirmMessage = productCount > 0 || userCount > 0
  ...
  const getTypeLabel = (type?: BusinessUnitType | string) => {
    ...
    return (
      <div className="p-6 text-center">
        <div className="max-w-md mx-auto">
          ...
          <h2 className="text-xl font-semibold text-gray-700 dark:text-gray-300">Business Unit Not Found</h2>
          <p className="text-gray-500 dark:text-gray-400 mt-2">The business unit you're looking for doesn't exist or has been removed.</p>
          <button
    ...
    <div className="p-6 max-w-6xl mx-auto">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">{unit.name}</h1>
            ...
            <div className="flex items-center gap-4 mt-1">
              ...
              {unit.type && (
                <p className="text-gray-600 dark:text-gray-400">
                  Type: <span className="font-medium">{getTypeLabel(unit.type)}</span>
                </p>
              )}
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Business Unit Information</h3>
          <div className="space-y-3">
            <div className="flex items-center gap-3 text-sm">
              <Building className="w-4 h-4 text-gray-400 flex-shrink-0" />
              <span className="text-gray-600 dark:text-gray-400">Company:</span>
              <span className="font-medium text-gray-900 dark:text-white">{unit.company?.name || 'N/A'}</span>
            </div>
            {unit.address && (
              <div className="flex items-center gap-3 text-sm">
                <MapPin className="w-4 h-4 text-gray-400 flex-shrink-0" />
                <span className="text-gray-900 dark:text-white">{unit.address}</span>
              </div>
            )}
            {unit.phone && (
              <div className="flex items-center gap-3 text-sm">
                <Phone className="w-4 h-4 text-gray-400 flex-shrink-0" />
                <a href={`tel:${unit.phone}`} className="text-blue-600 hover:underline">
                  {unit.phone}
                </a>
              </div>
            )}
            {unit.email && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <a href={`mailto:${unit.email}`} className="text-blue-600 hover:underline">
                  {unit.email}
                </a>
              </div>
            )}
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <h3 className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-4">Quick Statistics</h3>
          <div className="grid grid-cols-2 gap-3">
            <div className="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">
                {stats?.totalEmployees || unit._count?.userBusinessUnits || unit._count?.users || 0}
              </p>
            </div>
            ...
            <div className="p-3 bg-purple-50 dark:bg-purple-900/20 rounded-lg">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats?.sales || 0}</p>
            </div>
          ...
          {(stats?.lowStockItems !== undefined || stats?.outOfStockItems !== undefined) && (
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 grid grid-cols-2 gap-3">
              {stats?.lowStockItems !== undefined && (
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="font-medium text-orange-600 dark:text-orange-400">{stats.lowStockItems}</span>
                </div>
              )}
              {stats?.outOfStockItems !== undefined && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Out of Stock:</span>
                  <span className="font-medium text-red-600 dark:text-red-400">{stats.outOfStockItems}</span>
                </div>
              )}
            </div>
          )}
          
          {(stats?.monthlyRevenue !== undefined || stats?.monthlySales !== undefined) && (
            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700 grid grid-cols-2 gap-3">
              ...
              {stats?.monthlySales !== undefined && (
                <div className="flex items-center justify-between text-sm">
                  <span className="text-gray-500 dark:text-gray-400">Monthly Sales:</span>
                  <span className="font-medium text-blue-600 dark:text-blue-400">{stats.monthlySales}</span>
                </div>
              )}
            </div>
          )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
              <span className="text-gray-500 dark:text-gray-400">Business Unit ID</span>
              <span className="font-mono text-gray-600 dark:text-gray-300">{unit.id}</span>
            </div>
            <div className="flex justify-between py-2 border-b border-gray-100 dark:border-gray-700">
              <span className="text-gray-500 dark:text-gray-400">Type</span>
              <span className="font-medium text-gray-900 dark:text-white">{getTypeLabel(unit.type)}</span>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <Link href={`/admin/business-units/${unit.id}/users`} className="text-sm text-blue-600 hover:underline">View All</Link>
          </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-sm font-medium text-gray-900 dark:text-white">
                        {user.user ? `${user.user.firstName || ''} ${user.user.lastName || ''}`.trim() || 'Unknown User' : user.userId}
                      </p>
                      <p className="text-xs text-gray-500 dark:text-gray-400">{user.role}</p>
                    </div>
                  </div>
                  <span className={`text-xs px-2 py-0.5 rounded-full ${user.isActive ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-400'}`}>
                    {user.isActive ? 'Active' : 'Inactive'}
                  </span>
                </div>
              ))}
              {unit.users.length > 5 && (
                <p className="text-sm text-gray-500 dark:text-gray-400 text-center pt-2">+{unit.users.length - 5} more users</p>
              )}
            </div>
          ) : (
            <p className="text-gray-500 dark:text-gray-400 text-sm">No users assigned to this business unit.</p>
          )}
        </div>
      </div>
      ...
      <div className="mt-6 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href={`/admin/business-units/${unit.id}/products`} className="text-sm text-blue-600 hover:underline">View All Products</Link>
        </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {unit.products.slice(0, 6).map((product) => (
              <div key={product.id} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
                ...
                <p className="text-sm font-semibold text-gray-900 dark:text-white mt-1">{formatCurrency(product.unitPrice)}</p>
              </div>
            ))}
            {unit.products.length > 6 && (
              <div className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg flex items-center justify-center">
                <p className="text-sm text-gray-500 dark:text-gray-400">+{unit.products.length - 6} more</p>
              </div>
            )}
          </div>
        ) : (
          <p className="text-gray-500 dark:text-gray-400 text-sm">No products in this business unit.</p>
        )}
      </div>
    </div>
  );

// D:\Projects\Kalwanga\packages\web\src\components\orders\OrderForm.tsx
...
// ============================================
//
// Only methods whose flow can complete on the current page are
// exposed. Redirect-based gateways (PayPal, Flutterwave, Paystack)
// and Square require SDK integration that isn't wired yet, so they
...
interface PaymentMethodDef {
  ...
  /** When true, the flow shows an "amount tendered" input. */
  takesTender: boolean;
}

const PAYMENT_METHODS: readonly PaymentMethodDef[] = [
  {
    value: 'CASH',
    label: 'Cash',
    icon: DollarSign,
...
export function OrderForm({ mode = 'order' }: OrderFormProps) {
  ...
  // ── Cart ───────────────────────────────────────────────────
  const [cart, setCart] = useState<AuthCart | null>(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled && isMountedRef.current) setShift(current);
      } catch (err) {
        console.error('Failed to load shift:', err);
        if (!cancelled && isMountedRef.current) setShift(null);
      } finally {
        if (!cancelled && isMountedRef.current) setLoadingShift(false);
      }
    })();
    return () => {
      cancelled = true;
  ...
  // ============================================
  //
  // The cart is user-scoped, not shift-scoped. Load it as soon as the
  // shift check finishes so a non-cash checkout isn't blocked on
  // opening a shift.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        setCartLoading(true);
        const active = await cartService.getCart();
        if (!cancelled && isMountedRef.current) setCart(active);
      } catch (err: any) {
        ...
        if (!cancelled && isMountedRef.current) setCartLoading(false);
      }
    })();
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (isMountedRef.current) setSearchingCustomers(false);
      }
    }, 250);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      try {
        ...
        if (isMountedRef.current) setSearchingItems(false);
      }
    }, 250);
    return () => clearTimeout(t);
  }, [itemSearch]);
  ...
  /**
   * Prefill the tendered amount only for methods that use it, and
   * only when the field is currently empty.
   */
  useEffect(() => {
    if (!activeMethod.takesTender) return;
    if (totals.total > 0 && !paidAmount) {
      setPaidAmount(totals.total.toFixed(2));
    }
  }, [activeMethod.takesTender, totals.total, paidAmount]);
  ...
  const handleSelectCustomer = useCallback(
    async (c: CustomerOption | null) => {
      ...
      if (c === null) {
        // The backend does not support clearing a customer on a cart.
        // It would 400 on `associateCustomerSchema` because the body
        // requires a non-empty `customerId`. Surface that limitation
        // instead of silently pretending we cleared it.
        toast.info(
          'Removing a customer is not supported. Clear the cart or start a new one to detach the customer.',
        );
        return;
      }

      try {
        const updated = await cartService.associateCustomer(c.id);
        if (isMountedRef.current) setCart(updated);
      } catch (err: any) {
        if (isMountedRef.current) {
          toast.error(
            err?.response?.data?.message ||
              'Failed to attach customer to cart',
          );
        }
      }
    },
  ...
  const handleAddItem = useCallback(async (p: ProductOption) => {
    ...
    try {
      ...
      if (isMountedRef.current) setAddingItemId(null);
    }
  }, []);
  ...
  //
  // Flow:
  //   1. POST /orders                  → order in PENDING
  //   2. POST /orders/:id/convert-to-sale → sale, order becomes COMPLETED
  //   3. POST /payments                → payment against the sale
  // ============================================
  ...
  const handleCompleteSale = useCallback(async () => {
    ...
    if (activeMethod.takesTender && totals.paid < totals.total) {
    ...
    try {
      ...
      // ── 1. Create the order ─────────────────────────────────
      // `unitPrice` is intentionally omitted — the backend derives
      // it from the authoritative product / variant price.
      const order = await orderService.createOrder({
        ...
        discount: totals.extraDiscount || undefined,
      ...
      // ── 2. Convert order → sale ─────────────────────────────
      const sale = await orderService.convertOrderToSale(orderId);

      const saleId = sale.id;
      if (!saleId) {
        throw new Error('Order converted but no Sale ID was returned');
      }
      const receiptNumber = sale.receiptNumber;

      // ── 3. Record the payment against the Sale ──────────────
      //
      // `paymentMethod` is a narrow literal union, but
      // `paymentService.processOrderPayment` expects the even
      // narrower `PosPaymentMethod`. We narrow at each branch
      // rather than spreading a common object, so TS can prove
      // the literal is assignable.
      const description = `Sale ${
        receiptNumber || orderNumber || saleId
      }`;
      ...
      const basePaymentInput = {
        ...
        cashRegisterSessionId:
          shift?.cashRegisterSessionId ?? undefined,
        currency: 'USD',
        description,
        metadata: {
          source: mode,
      ...
      if (paymentMethod === 'MOBILE_MONEY' && mobilePhone.trim()) {
        ...
        // CASH (the only remaining member of PaymentMethodValue)
        await paymentService.processOrderPayment({
          ...basePaymentInput,
          paymentMethod: 'CASH',
        });
          ...
          <div className="bg-warning-50 dark:bg-warning-900/20 border border-warning-200 dark:border-warning-800 rounded-xl p-4 flex items-start gap-3 animate-slide-down">
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-warning-800 dark:text-warning-300 mt-1">
                Cash sales require an open shift. Open one, or switch
                to a non-cash payment method.
              </p>
            </div>
            <Link
              href="/admin/shifts"
              ...
              {PAYMENT_METHODS.map((m) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() =>
                      setPaymentMethod(m.value as PaymentMethodValue)
                    }

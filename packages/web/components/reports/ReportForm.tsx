...
export function ReportForm({ businessUnitId, companyId }: ReportFormProps) {
  ...
  return (
    <div className="card-brand shadow-soft p-6 space-y-6 animate-fade-in">
      <div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
          {REPORT_TYPES.map((type) => (
            <button
              ...
              className={`p-3 rounded-lg border text-left transition duration-250 focus-ring ${
            ...
            >
              ...
              <p className="text-xs text-gray-500 dark:text-gray-400 mt-1 line-clamp-2">{type.description}</p>
            </button>
          ))}
        </div>
        ...
        {reportType === 'sales' && (
          <div className="mb-6 animate-slide-down">
            <label className="block text-sm font-medium mb-1 text-gray-700 dark:text-gray-300">Group By</label>
            <select
              value={groupBy}
            ...
            {REPORT_FORMATS.map((f) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition duration-250 focus-ring ${
                    format === f.value

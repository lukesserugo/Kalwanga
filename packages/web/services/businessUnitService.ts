...
export function getBusinessUnitId(): string {
  try {
    ...
    if (stored && stored !== 'undefined' && stored !== 'null' && stored !== 'default') {
      return stored;
    }
  } catch (_e) {
    // Ignore
  ...
  try {
    ...
    if (userStr) {
      ...
      if (user?.businessUnits?.[0]?.businessUnitId && user.businessUnits[0].businessUnitId !== 'default') {
        return user.businessUnits[0].businessUnitId;
      }
      if (user?.businessUnits?.[0]?.id && user.businessUnits[0].id !== 'default') {
  ...
  try {
    ...
    if (sessionId && sessionId !== 'undefined' && sessionId !== 'null' && sessionId !== 'default') {
      return sessionId;
    }
  } catch (_e) {
    // Ignore
...
// Updated to accept CUIDs (Prisma default) and UUIDs
...
function extractArray<T>(response: unknown): T[] {
  ...
  if (hasSuccessProperty(response) && hasDataProperty(response)) {
    ...
    // Check for businessUnits array in response
    if (isObject(data) && 'businessUnits' in data && Array.isArray(data.businessUnits)) {
...
export const businessUnitService = {
  ...
  async getById(id: string): Promise<BusinessUnit> {
    try {
      ...
      console.error(`❌ Failed to fetch business unit ${id}:`, error);
      throw error;
    }
  },

  async create(data: Partial<BusinessUnit> & { type?: string }): Promise<BusinessUnit> {
    try {
      if (!data.name?.trim()) {
  ...
  async update(id: string, data: Partial<BusinessUnit> & { type?: string }): Promise<BusinessUnit> {
    try {
      if (!id || id === 'default') {
        throw new Error('Business unit ID is required');
      }
      ...
      console.error(`❌ Failed to update business unit ${id}:`, error);
      throw error;
    }
  },
  ...
  async delete(id: string): Promise<{ message: string }> {
    try {
      ...
      if (hasDataProperty(response) && isObject(response.data) && hasMessageProperty(response.data)) {
      ...
      console.error(`❌ Failed to delete business unit ${id}:`, error);
      throw error;
    }
  },
  ...
  async bulkDelete(ids: string[]): Promise<BulkDeleteResult> {
    try {
      ...
      const invalidIds = ids.filter(id => id !== 'default' && !isValidID(id));
      ...
      return {
        ...
        totalProcessed: result.totalProcessed || (result.deletedCount || 0) + (result.softDeletedCount || 0),
      };
    } catch (error) {
      console.error('❌ Failed to bulk delete business units:', error);
      throw error;
    }
  },
  ...
  async getStats(id: string): Promise<BusinessUnitStats> {
    try {
      ...
      console.error(`❌ Failed to fetch stats for business unit ${id}:`, error);
      throw error;
    }
  },

  async getBusinessUnitUsers(businessUnitId: string): Promise<BusinessUnitUser[]> {
    try {
      if (!businessUnitId || businessUnitId === 'default') {
        throw new Error('Business unit ID is required');
      }
      
      const response = await api.get(`/business-units/${businessUnitId}/users`);
      return extractArray<BusinessUnitUser>(response);
    } catch (error) {
      console.error(`❌ Failed to fetch users for business unit ${businessUnitId}:`, error);
      throw error;
    }
  },
  ...
  async addUserToBusinessUnit(
    ...
    try {
      ...
      const response = await api.post(`/business-units/${businessUnitId}/users`, {
        userId,
        role,
      });
      ...
      console.error('❌ Failed to add user to business unit:', error);
      throw error;
    }
  },
  ...
  async removeUserFromBusinessUnit(
    ...
    try {
      ...
      const response = await api.delete(`/business-units/${businessUnitId}/users/${userId}`);
      ...
      console.error('❌ Failed to remove user from business unit:', error);
      throw error;
    }
  },

  async getOrCreateDefaultBusinessUnit(companyId: string): Promise<BusinessUnit> {
    try {
      ...
      if (!result || !hasIdProperty(result)) {
        throw new Error('Failed to get default business unit: Invalid response');
      }
      ...
      console.error('❌ Failed to get or create default business unit:', error);
      throw error;
    }
  },
  ...
  async getByCompany(companyId: string): Promise<BusinessUnit[]> {
    try {
      ...
      console.error(`❌ Failed to fetch business units for company ${companyId}:`, error);
      throw error;
    }
  },

  async ensureUserBusinessUnit(userId: string, companyId: string): Promise<BusinessUnit> {
    try {
      if (!userId) {
        throw new Error('User ID is required');
      }
      ...
      const response = await api.post('/business-units/ensure', { userId, companyId });
      const result = extractData<BusinessUnit>(response);
      
      if (!result || !hasIdProperty(result)) {
        throw new Error('Failed to ensure user business unit: Invalid response');
      }
      ...
      console.error('❌ Failed to ensure user business unit:', error);
      throw error;
    }
  },

  async getByCode(code: string, companyId?: string): Promise<BusinessUnit | null> {
    try {
      if (!code) {
        throw new Error('Business unit code is required');
      }
      
      const params: Record<string, string> = { code: code.toUpperCase() };
      if (companyId) params.companyId = companyId;
      
      const response = await api.get('/business-units/code', { params });
      const result = extractData<BusinessUnit>(response);
      ...
      console.error(`❌ Failed to fetch business unit by code ${code}:`, error);
      return null;
    }
  },
  ...
  async getWithDetails(id: string): Promise<BusinessUnitDetails> {
    try {
      ...
      if (!result || !hasIdProperty(result)) {
        throw new Error('Failed to get business unit details: Invalid response');
      }
      ...
      console.error(`❌ Failed to fetch business unit details ${id}:`, error);
      throw error;
    }
  },
  ...
  async createBusinessUnit(data: Partial<BusinessUnit> & { type?: string }): Promise<BusinessUnit> {
    return this.create(data);
  },

  async updateBusinessUnit(id: string, data: Partial<BusinessUnit> & { type?: string }): Promise<BusinessUnit> {
  ...
  async addUser(businessUnitId: string, userId: string, role: string): Promise<BusinessUnitUser> {
    return this.addUserToBusinessUnit(businessUnitId, userId, role);
  },

  async removeUser(businessUnitId: string, userId: string): Promise<{ message: string }> {

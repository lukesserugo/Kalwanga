...
import {
  ...
  Phone, MapPin, Eye, Loader2, Lock, Grid, List,
...
export default function CustomersPage() {
  ...
  // Used to force-refetch when returning from create/edit.
  const [refreshKey, setRefreshKey] = useState(0);

  // Permissions
  const canViewCustomers =
    canView(PermissionResource.CUSTOMER) || canManage(PermissionResource.CUSTOMER);
  const canCreateCustomers =
    canCreate(PermissionResource.CUSTOMER) || canManage(PermissionResource.CUSTOMER);
  const canEditCustomers =
    canEdit(PermissionResource.CUSTOMER) || canManage(PermissionResource.CUSTOMER);
  const canDeleteCustomers =
    canDelete(PermissionResource.CUSTOMER) || canManage(PermissionResource.CUSTOMER);

  // ── Fetch customers ────────────────────────────────────────
  // ✅ We deliberately DO NOT send companyId. The backend derives it
  //    from the authenticated user. Sending 'default' filters out
  //    every row because no customer has companyId = 'default'.
  const loadCustomers = useCallback(
    async (showLoading = true) => {
      ...
      try {
        ...
        const customersData: Customer[] = Array.isArray(result?.data)
          ? result.data
          : [];
        const total = result?.total ?? customersData.length;
        const totalPages =
          result?.totalPages ?? Math.max(1, Math.ceil(total / pagination.limit));
    ...
    // ✅ Only depend on things that actually change the query.
    //    Do NOT include pagination.total / totalPages (they're derived).
    [
      canViewCustomers,
      pagination.page,
      pagination.limit,
  ...
  // Refetch when the create page navigates back
  useEffect(() => {
  ...
  // ── Client-side sort (search + status already filtered server-side) ──
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-1">
            {pagination.total || customers.length} customers • Manage your
              ...
              >
                ...
                <div className="mt-3 pt-3 border-t flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span
                      className={`px-2 py-0.5 rounded-full text-xs ${tier.color}`}
                    >
                      {tier.label}
                    </span>
                ...
                {sortedCustomers.map((customer) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`px-2 py-1 rounded-full text-xs ${tier.color}`}
                        >
                          {tier.label}
                        </span>

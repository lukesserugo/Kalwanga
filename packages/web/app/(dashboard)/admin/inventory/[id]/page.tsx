...
import { inventoryService, Inventory, InventoryTransaction, InventoryItemResponse } from '../../../../../services/inventoryService';
...
const getStockStatus = (available: number, reorderPoint: number) => {
  if (available <= 0) return { status: 'out_of_stock', label: 'Out of Stock', color: 'red' };
  if (available <= reorderPoint) return { status: 'low_stock', label: 'Low Stock', color: 'yellow' };
  return { status: 'in_stock', label: 'In Stock', color: 'green' };
};

const getStockStatusColor = (status: string) => {
  switch (status) {
    case 'out_of_stock': return 'bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-300';
    case 'low_stock': return 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-300';
    case 'in_stock': return 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300';
    default: return 'bg-gray-100 dark:bg-gray-700/50 text-gray-700 dark:text-gray-300';
  }
};

const getTransactionTypeColor = (type: string) => {
  const additions = ['PURCHASE', 'ADJUSTMENT_IN', 'RETURN', 'INITIAL', 'RESTOCK', 'TRANSFER_IN'];
  const removals = ['SALE', 'ADJUSTMENT_OUT', 'ISSUE', 'DAMAGED', 'LOST', 'TRANSFER_OUT'];
...
export default function InventoryDetailPage() {
  ...
  const fetchInventoryItem = useCallback(async () => {
    ...
    try {
      ...
      let stats = { totalIn: 0, totalOut: 0, netChange: 0 };
        ...
        if (transResponse) {
          ...
          transactionData.forEach((t: InventoryTransaction) => {
            ...
            if (type === 'PURCHASE' || type === 'RESTOCK' || type === 'RETURN' || 
                type === 'ADJUSTMENT_IN' || type === 'INITIAL' || type === 'TRANSFER_IN') {
      ...
      // Build the full item object with all fields
      const fullItem: InventoryItemWithDetails = {
        id: inventoryData.id || id,
        productId: inventoryData.productId || inventoryData.product?.id,
        name: inventoryData.name || inventoryData.product?.name || 'Unknown Item',
        sku: inventoryData.sku || inventoryData.product?.sku || 'N/A',
        barcode: inventoryData.barcode || inventoryData.product?.barcode || null,
        unitPrice: inventoryData.unitPrice || inventoryData.product?.unitPrice || 0,
        ...
        reorderQuantity: inventoryData.reorderQuantity || inventoryData.maxStock || 10,
        businessUnitId: inventoryData.businessUnitId,
        createdAt: inventoryData.createdAt,
        updatedAt: inventoryData.updatedAt,
        images: (inventoryData.images && inventoryData.images.length > 0) ? 
          inventoryData.images : (inventoryData.product?.images || []),
        description: inventoryData.description || inventoryData.product?.description || null,
        weight: inventoryData.weight ?? inventoryData.product?.weight ?? 0,
        taxRate: inventoryData.taxRate ?? inventoryData.product?.taxRate ?? 0,
        tags: (inventoryData.tags && inventoryData.tags.length > 0) ? 
          inventoryData.tags : (inventoryData.product?.tags || []),
        isActive: inventoryData.isActive ?? inventoryData.product?.isActive ?? true,
        isDigital: inventoryData.isDigital ?? inventoryData.product?.isDigital ?? false,
        featured: inventoryData.featured ?? inventoryData.product?.featured ?? false,
        category: inventoryData.category || inventoryData.product?.category?.name || 'Uncategorized',
        categoryId: inventoryData.categoryId || inventoryData.product?.category?.id || null,
        supplier: inventoryData.supplier || inventoryData.product?.supplier?.name || null,
        supplierId: inventoryData.supplierId || inventoryData.product?.supplier?.id || null,
      ...
      if (fullItem.businessUnitId) {
        try {
          ...
          if (stored) {
            ...
            const bu = units.find((u: any) => u.id === fullItem.businessUnitId);
            if (bu) {
      ...
      const errorMessage = error?.response?.data?.message || error?.message || 'Failed to load inventory item';
  ...
  const handleUpdateStock = async () => {
    ...
    try {
      ...
      const isAddition = ['ADJUSTMENT_IN', 'PURCHASE', 'RESTOCK', 'RETURN'].includes(stockUpdate.transactionType);
      const isRemoval = ['ADJUSTMENT_OUT', 'SALE', 'ISSUE'].includes(stockUpdate.transactionType);
      ...
      // METHOD 1: Try using the update endpoint
      try {
        console.log('📤 Trying inventoryService.updateItem...');
        const response = await inventoryService.updateItem(item.id, {
          quantity: newQuantity,
          location: item.location,
          minStock: item.reorderPoint,
        });
        console.log('📥 Update response (updateItem):', response);
        updateSuccess = true;
      } catch (err: any) {
        console.warn('inventoryService.updateItem failed:', err?.response?.status);
        lastError = err;
      }

      // METHOD 2: Try PATCH /inventory/items/{id}
      if (!updateSuccess) {
        try {
          console.log('📤 Trying PATCH /inventory/items/{id}...');
      ...
      // METHOD 3: Try PUT /inventory/items/{id}
      if (!updateSuccess) {
        try {
          console.log('📤 Trying PUT /inventory/items/{id}...');
          const response = await api.put(`/inventory/items/${item.id}`, {
            quantity: newQuantity,
            location: item.location,
            reorderPoint: item.reorderPoint,
          });
      ...
      // METHOD 4: Try POST /inventory/items/{id}/adjust
      if (!updateSuccess) {
        try {
          console.log('📤 Trying POST /inventory/items/{id}/adjust...');
          const response = await api.post(`/inventory/items/${item.id}/adjust`, {
      ...
      // METHOD 5: Try POST /inventory/items/{id}/stock
      if (!updateSuccess) {
        try {
          console.log('📤 Trying POST /inventory/items/${item.id}/stock...');
          const response = await api.post(`/inventory/items/${item.id}/stock`, {
            quantity: stockUpdate.quantity,
            transactionType: stockUpdate.transactionType,
            notes: stockUpdate.notes,
          });
          console.log('📥 POST /stock response:', response);
          updateSuccess = true;
        } catch (err: any) {
          console.warn('POST /stock failed:', err?.response?.status);
          lastError = err;
        }
      }

      // METHOD 6: Try updating via product if inventory update fails
      if (!updateSuccess && item.productId) {
        try {
          console.log('📤 Trying productService.updateProduct...');
          const response = await inventoryService.updateStock(item.id, {
            quantity: stockUpdate.quantity,
            transactionType: stockUpdate.transactionType,
            notes: stockUpdate.notes,
          });
          ...
          console.warn('productService.updateProduct failed:', err?.response?.status);
          lastError = err;
        }
      }

      // METHOD 7: Try direct stock update via inventoryService.updateStock
      if (!updateSuccess) {
        try {
          console.log('📤 Trying inventoryService.updateStock...');
          const response = await inventoryService.updateStock(item.id, {
            quantity: stockUpdate.quantity,
            transactionType: stockUpdate.transactionType,
            notes: stockUpdate.notes,
          ...
          console.warn('inventoryService.updateStock failed:', err?.response?.status);
          lastError = err;
        }
      }
      ...
      if (error?.response?.status === 404) {
        ...
        errorMsg = error?.response?.data?.message || error?.message || 'Failed to update stock';
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const errorMsg = error?.response?.data?.message || 'Failed to delete inventory item';
      setError(errorMsg);
      toast.error(errorMsg);
    } finally {
      setDeleting(false);
  ...
  const generateQRCode = async () => {
    ...
    // ✅ FIX: Reset error state properly
    setError(null);
    ...
    // Try productService.generateQRCode first
    try {
      ...
      const fallbackUrl = `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(JSON.stringify(qrData))}`;
      setQrCodeUrl(fallbackUrl);
      setShowQRCode(true);
      
    } catch (error) {
      console.error('Failed to generate QR code:', error);
      // ✅ FIXED: Set error to a proper string message instead of {}
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-8">
        <div className="text-center">
          ...
          <p className="text-gray-500 dark:text-gray-400 mt-2">The inventory item you're looking for doesn't exist or has been removed.</p>
          <button
  ...
  const stockStatus = getStockStatus(availableStock, item.reorderPoint || 5);
  const stockStatusColor = getStockStatusColor(stockStatus.status);

  const images = (item.images && item.images.length > 0) ? item.images : (item.product?.images || []);
  const hasImages = images.length > 0;
  const tags = (item.tags && item.tags.length > 0) ? item.tags : (item.product?.tags || []);
    ...
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-6">
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-3 flex-wrap">
              ...
              <span className={`px-2.5 py-0.5 text-xs font-medium rounded-full ${stockStatusColor} flex-shrink-0`}>
                {stockStatus.label}
              </span>
              {!item.isActive && (
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          ...
          <p className="text-xs text-gray-400">Total Value: {formatCurrency((item.unitPrice || 0) * (item.quantity || 0))}</p>
        </div>
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-4">
          <p className="text-sm text-gray-500 dark:text-gray-400">Net Change</p>
          <p className={`text-2xl font-bold ${
            (item.stats?.netChange || 0) >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
          }`}>
            {(item.stats?.netChange || 0) >= 0 ? '+' : ''}{item.stats?.netChange || 0}
          </p>
          <p className="text-xs text-gray-400">In: {item.stats?.totalIn || 0} | Out: {item.stats?.totalOut || 0}</p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <p className="font-mono text-gray-900 dark:text-white">{item.sku || 'N/A'}</p>
                {item.sku && (
                  <button
            ...
            <div>
              ...
              <p className="text-gray-900 dark:text-white">{item.category || 'Uncategorized'}</p>
            </div>
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Supplier</p>
              <p className="text-gray-900 dark:text-white">{item.supplier || 'N/A'}</p>
            </div>
            {item.expiryDate && (
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-gray-900 dark:text-white">{formatDate(item.expiryDate)}</p>
                </div>
              </div>
            )}
            {item.batchNumber && (
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-gray-900 dark:text-white">{item.batchNumber}</p>
                </div>
              </div>
            )}
            ...
            <div className="sm:col-span-2">
              ...
              <div className="flex flex-wrap gap-1.5">
                {tags.length > 0 ? (
                  tags.map((tag, index) => (
                    <span key={index} className="px-2.5 py-0.5 bg-gray-100 dark:bg-gray-700 rounded-full text-xs text-gray-600 dark:text-gray-300 flex items-center gap-1">
          ...
          {item.barcode && (
            <button
              onClick={() => {
                ...
                const barcodeUrl = `https://barcode.tec-it.com/barcode.ashx?data=${encodeURIComponent(barcodeValue)}&code=EAN-13&dpi=96`;
                ...
                {transactions.slice(0, 50).map((transaction) => (
                  <tr key={transaction.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="px-4 py-3">
                      <span className={`inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium ${getTransactionTypeColor(transaction.transactionType)}`}>
                        {getTransactionIcon(transaction.transactionType)}
                        {transaction.transactionType}
                      </span>
                    </td>
                    <td className={`px-4 py-3 font-medium ${transaction.quantity > 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setStockUpdate({ ...stockUpdate, quantity: Math.max(1, parseInt(e.target.value) || 0) })}
                    min="1"
                    className="w-full px-4 py-2.5 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
                  />
                </div>
                ...
                <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/30 p-3 rounded-lg">
                  ...
                  <span>Current stock: <strong>{item.quantity || 0}</strong></span>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <p className="text-center text-xs text-gray-400 dark:text-gray-500">
                SKU: {item.sku || 'N/A'} • Price: {formatCurrency(item.unitPrice || 0)}
              </p>
                ...
                <button
                  onClick={() => {
                    if (qrCodeUrl) {
                      ...
                      link.href = qrCodeUrl;
                      link.download = `qrcode-${(item.sku || 'item').toLowerCase()}.png`;

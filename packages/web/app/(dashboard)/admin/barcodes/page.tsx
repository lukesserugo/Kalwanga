...
import { 
  Barcode, QrCode, Search, RefreshCw, 
  Loader2, Download, Printer, Copy, Check,
  Package, Eye, ArrowLeft, Plus
} from 'lucide-react';
...
export default function BarcodesPage() {
  ...
  const handleBulkGenerate = async () => {
    try {
      ...
      toast.success(`Generated ${result.generated} barcodes, ${result.failed} failed`);
      await loadProducts();
    } catch (error: any) {
  ...
  if (!canViewProducts) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-2">You don't have permission to view barcodes.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-6">
            ...
            <table className="w-full">
              <thead className="bg-gray-50 dark:bg-gray-700/50 border-b border-gray-200 dark:border-gray-700">
                <tr>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">QR Code</th>
                ...
                {filteredProducts.map((product) => (
                  <tr key={product.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      {product.barcode ? (
                        <div className="flex items-center gap-2">
                          <span className="font-mono text-sm text-gray-900 dark:text-white">{product.barcode}</span>
                          <button
                            onClick={() => {
                        ...
                        {product.barcode && (
                          <>
                            <button
                              onClick={() => {
                                // Download barcode
                                barcodeService.getBarcodeImage(product.id).then((data) => {
                            ...
                            <button
                              onClick={() => {
                                // Print barcode
                                window.open(`/admin/barcodes/print/${product.id}`, '_blank');
                              }}
                              className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors"
                              title="Print Barcode"
                            >
      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-md w-full max-h-[90vh] overflow-y-auto p-6">
            <ProductBarcode

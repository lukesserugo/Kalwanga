...
const StatsCard: React.FC<{
  ...
  const colorClasses: Record<string, { bg: string; text: string }> = {
    brand: { bg: 'bg-brand-50 dark:bg-brand-900/20', text: 'text-brand-600 dark:text-brand-400' },
    'brand-accent': { bg: 'bg-brand-accent-50 dark:bg-brand-accent-900/20', text: 'text-brand-accent-600 dark:text-brand-accent-400' },
    ...
    danger: { bg: 'bg-danger-50 dark:bg-danger-900/20', text: 'text-danger-600 dark:text-danger-400' },
  };
    ...
    >
      ...
      {change !== undefined && change !== 0 && (
        <div className="flex items-center gap-1 mt-3">
          ...
          <span className={`text-sm font-medium tabular-nums ${isPositive ? 'text-success-500' : 'text-danger-500'}`}>
            {Math.abs(change)}%
          </span>
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const statusConfig: Record<string, { label: string; color: string; icon: React.ElementType }> = {
    ...
    PROCESSING: { label: 'Processing', color: 'bg-brand-100 text-brand-700 dark:bg-brand-900/30 dark:text-brand-300', icon: Loader2 },
    CANCELLED: { label: 'Cancelled', color: 'bg-danger-100 text-danger-700 dark:bg-danger-900/30 dark:text-danger-300', icon: X },
              ...
              {recentSales.slice(0, 10).map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors cursor-pointer" onClick={() => handleViewSale(sale)}>
                  ...
                  <td className="px-4 sm:px-6 py-4 whitespace-nowrap text-right" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded transition-colors ml-2 focus-ring">
                    ...
                    {selectedSale.items?.map((item: any) => (
                      <div key={item.id} className="flex justify-between items-center p-2 border border-gray-200 dark:border-gray-700 rounded-lg">
                        <div>
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400 tabular-nums">× {item.quantity}</p>
                        </div>
                        <span className="font-bold text-gray-900 dark:text-white tabular-nums">{formatCurrency(item.total)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
                  <div className="space-y-1 max-w-xs ml-auto">
                    ...
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="tabular-nums">{formatCurrency(selectedSale.tax)}</span>
                    </div>
                    ...
                    <div className="flex justify-between font-bold text-lg pt-2 border-t border-gray-200 dark:border-gray-700">
                      <span>Total</span>
                      <span className="text-brand-600 tabular-nums">{formatCurrency(selectedSale.total)}</span>
                    </div>
                  </div>
                </div>
                <div className="flex gap-3 pt-4 border-t border-gray-200 dark:border-gray-700">
                  <button className="px-4 py-2 bg-brand-500 text-white rounded-lg hover:bg-brand-600 flex items-center gap-2 focus-ring">
                    <Printer className="w-4 h-4" /> Print
                  </button>
                  <button onClick={() => setShowDetailModal(false)} className="px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 focus-ring">
                    Close
                  </button>
                </div>
              </div>

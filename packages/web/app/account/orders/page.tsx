...
export default function AccountOrdersPage() {
  ...
  const loadOrders = useCallback(
    ...
    [pagination.page, pagination.limit, statusFilter, appliedSearch, user?.id]
  );

  // Load on mount and when the page / status filter changes
  useEffect(() => {
    loadOrders();
  }, [pagination.page, statusFilter, appliedSearch]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const getProviderConfig = (providerCode: string) =>
    ...
    { icon: '💳', name: providerCode || 'Payment', color: 'gray' };
      ...
      <div className="max-w-6xl mx-auto px-4 py-8 mt-20">
        ...
        >
          <div className="flex flex-wrap items-center gap-3">
            ...
            {(searchQuery || appliedSearch || statusFilter !== 'all') && (
              <button
          ...
          <div className="space-y-4">
            {orders.map((order, index) => {
              const providerConfig = getProviderConfig(
                order.payment?.provider || order.payment?.gatewayId || ''
              );
                ...
                <div className="flex flex-wrap items-center gap-4">
                  ...
                  {selectedOrder.payment && (
                    <div className="flex items-center gap-2">
                      ...
                      >
                        Paid: {formatCurrency(selectedOrder.payment.amount)} via{' '}
                        {getPaymentMethodLabel(

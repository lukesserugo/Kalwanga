...
import {
  CheckCircle, Receipt, Download, Printer, Share2,
  ShoppingBag, ArrowRight, Clock, Mail, Phone,
  MapPin, User, Calendar, DollarSign, CreditCard,
  Package, Truck, Shield, Check, Copy, ExternalLink,
  Loader2, AlertCircle, Home, FileText, Star,
  Lock, Globe, Smartphone, Banknote, Wallet, Building, Gift
} from 'lucide-react';
...
export default function PaymentSuccessPage() {
  ...
  const fetchPaymentDetails = async () => {
    try {
      setLoading(true);
      // Fetch payment details using the session ID or payment intent ID
  ...
  const providerConfig = getProviderConfig(payment.provider || payment.gatewayId || '');
    ...
    <div className={`min-h-screen ${isDark ? 'dark bg-gray-950' : 'bg-gray-50'} transition-colors`}>
      ...
      <div className="max-w-4xl mx-auto px-4 py-8 mt-20">
        ...
        >
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div className="text-center">
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <p className={`text-lg font-semibold ${isDark ? 'text-white' : 'text-gray-900'} capitalize`}>
            ...
            {payment.sale?.items && payment.sale.items.length > 0 && (
              <div className="space-y-3 max-h-64 overflow-y-auto">
                {payment.sale.items.map((item, index) => (
                  <div key={index} className={`flex items-center gap-4 py-3 border-b ${isDark ? 'border-gray-700' : 'border-gray-100'}`}>
          ...
          {/* Actions */}
          <div className={`p-6 ${isDark ? 'bg-gray-700/30' : 'bg-gray-50'} border-t ${isDark ? 'border-gray-700' : 'border-gray-200'} flex flex-wrap gap-3`}>
            <Link
              href="/shop"
          ...
          {/* Auto-redirect */}
          {countdown > 0 && (
            <div className={`p-4 text-center text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'} border-t ${isDark ? 'border-gray-700' : 'border-gray-200'}`}>
